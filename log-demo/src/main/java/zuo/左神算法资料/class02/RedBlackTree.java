...
public class RedBlackTree extends AbstractSelfBalancingBinarySearchTree {
        ...
        while (x != root && isBlack(x)) {
            ...
            if (x == x.parent.left) {
                ...
                if (isRed(w)) { // case 1 - sibling is red
                    ...
                    rotateLeft(x.parent);
                    w = (RedBlackNode)x.parent.right; // converted to case zuo.2, 3 or 4
                }
                // case zuo.2 sibling is black and both of its children are black
                ...
                if (isRed(w)) { // case 1 - sibling is red
                    ...
                    w = (RedBlackNode)x.parent.left; // converted to case zuo.2, 3 or 4
                }
                // case zuo.2 sibling is black and both of its children are black
                if (isBlack(w.left) && isBlack(w.right)) {
                    w.color = ColorEnum.RED;
     ...
     * break only zuo.2 properties: root is red or if node is red then children must
     * be black.
     */
    private void insertRBFixup(RedBlackNode currentNode) {
        ...
        while (currentNode.parent != root && ((RedBlackNode) currentNode.parent).color == ColorEnum.RED) {
            ...
            if (parent == grandParent.left) {
                ...
                // case zuo.2/3 uncle is black - then we perform rotations
                else {
                    if (currentNode == parent.right) { // case zuo.2, first rotate left
                        currentNode = parent;
                ...
                // case zuo.2/3 uncle is black - then we perform rotations
                    ...
                    if (currentNode == parent.left) { // case zuo.2, first rotate right
                        currentNode = parent;
