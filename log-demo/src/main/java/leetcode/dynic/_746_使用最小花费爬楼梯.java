...
public class _746_使用最小花费爬楼梯 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < nums.length; i++) {
            System.out.println(minCostClimbingStairs22(nums[i]) == res[i]);
        }
    }

    public static int minCostClimbingStairs22(int[] cost) {
        int fn1 = 0, fn2 = 0, fn = 2;
        for (int i = 2; i <= cost.length; i++) {
            fn = Math.min(fn1 + cost[i - 1], fn2 + cost[i - 2]);
            fn2 = fn1;
            fn1 = fn;
        }
        return fn;
